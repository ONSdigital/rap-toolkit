...
**Stages:** Stages should use a functional style. A `stage` can be a file or a callable item, such as a function. File stages should define an entrypoint function that runs the stage; files without an entrypoint can use subprocess fallback, but the package has less control over that execution mode.
...
**Editable install:** This installs an editable version of the package. This means that when you update the package code you do not have to reinstall it for the changes to take effect. This saves a lot of time when you test your code.
...
**Codebase and samples:** Unless stated otherwise, the codebase is released under the MIT License. This covers both the codebase and any sample code in the documentation.

**Documentation:** The documentation is © Crown copyright and available under the terms of the Open Government 3.0 licence.
